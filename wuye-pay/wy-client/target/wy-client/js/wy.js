...
//		}
//		$.ajax({
//			url : "/wy-server/htmlAction/getLM.htm",
//			type : "post",
//			data : JSONWYID,
//			async : false,
//			success : function(data){
//				JFYF = data.lastMonth;
//				yearValue=JFYF.substr(0,4);
//				monthValue = JFYF.substr(4,2);
//				var param={
//						"feetype":feetype,
//						"WYID":WYID,
//						"HTID":HTID,
...
//		})
		if(url==null || url==""){
			requestUrl = "/wy-client/index.html?x0="+feetype+"&x1="+WYID+"&x2="+HTID+"&x3="+JFYF+"&x4="+source+"&SIGN="+SIGN+"&fromtype="+from;
		}else{
			requestUrl = url;
		}
	...
	function doAction(WYID, HTID, JFYF, source, SIGN, url) {
		var param = {
			"feetype":feetype,
			"WYID":WYID,
			"HTID":HTID,
		...
		for (var i=0;i<length;i++) {
			...
			if(finished == "true"){
				...
				var txt2 = "<input type='checkbox' name='mov' id='mov"+i+"' checked=true onclick='changeState("+i+")' onchange='showDetail("+i+");'  value='"+data[i].欠收合计+"'/>"
				   ...
				   +"<span id='finished"+i+"'>"+"</span><span id='jmglBZID"+i+"' value='"+data[i].BZID+"'></span></a><span style='float:right'>预收<select id='s"+i+"'   onchange='calculation(this.id,"+data[i].月费用+","+data[i].欠收合计+")'></select>个月</span>"
				   ...
				   +"<span class='ui-btn-inner' style='padding-top:4px;padding-bottom: 4px;'>欠收往月：<span id='qswy"+i+"'>"+data[i].欠收往月+"</span></span>"
		...
		for (var j=0;j<length;j++) {
			...
			if(finished == "false"){
				...
				var txt22 = "<input type='checkbox' name='mov' id='mov"+j+"' checked=true onclick='changeState("+j+")' onchange='showDetail("+j+");' "+disableFlag+" value='"+data[j].欠收合计+"'/>"
					...
					+"<span id='finished"+j+"'>"+"</span><span id='jmglBZID"+j+"' value='"+data[j].BZID+"'></span></a><span style='float:right'>预收<select id='s"+j+"'  onchange='calculation(this.id,"+data[j].月费用+","+data[j].欠收合计+")'></select>个月</span>"
					...
					+"<span class='ui-btn-inner' style='padding-top:4px;padding-bottom: 4px;'>欠收往月：<span id='qswy"+j+"'>"+data[j].欠收往月+"</span></span>"
...
//			for(var i=0;i<len;i++){
			...
			if($("#mov"+j).is(":checked")&&$("#mov"+j).attr("value")>0){
				if (j > 0) {
					jmglBZID += ",";
					jmglfeehj += ",";
				}
//				alert($("#mov"+j).next().next().children().val()==0)
//				if($("#mov"+j).next().next().children().val()==0){
//					jmglfeehj += 0;
//				}else{
//				}
				jmglBZID += $("#jmglBZID"+j).attr("value");
				jmglfeehj += $("#jmglfeehj"+j).html();
			} else {
...
//			}
		...
		if (jmglBZID.substr(0,1)==','){
			...
			jmglfeehj=jmglfeehj.substr(1);
		}
		list.jmglBZID = jmglBZID;
		list.jmglfeehj = jmglfeehj;
		
//		var flag = false;
//		var molen = list.jmglfeehj.split(",").length
//		for(var mi = 0;mi<molen;mi++){
//			var mo = $("#s"+mi).val();
//			if(mo != 0){
//				flag = false ; 
//				break;
...
//	}
	...
	function aiPay1() {
		...
		$.ajax({
			...
			async : false,
			success : function(data){
				if(data.lastMonth != JFYF){
					alert("请选择"+data.lastMonth.substr(0,4)+"年"+data.lastMonth.substr(4,2)+"月"+"进行缴费")
					return ;
				}else{
					var list = getList();
					var BZIDlist = list.jmglBZID;
					var feeList = list.jmglfeehj;
					...
					var price = $("#totalPrice").val();
					var redata =  serverOrder(price, HTID, waresid, BZIDlist,feeList);
	...
	function serverOrder(price, HTID,waresid, BZIDlist,feeList) {
...
//		}
	    ...
	    if ($("#checka").is(":checked")) {
	        ...
	        $("#detailList>[name=mov]:checkbox").prop("checked", false);
	        value=0.00;
	    }
	...
	function calculation(o,prefee,fee){
		var s = "#"+o;
		var jmglfeehjid = $(s).parent().prev().children().next().attr("id");
		var movid = $(s).parent().prev().prev().attr("id");
		...
		$.ajax({
			...
			success : function(data){
				if(data.lastMonth != JFYF){
					...
					serverOrder2(price, HTID, waresid, BZIDlist,feeList);
				}
			}
		})
	}

	function serverOrder2(price, HTID,waresid, BZIDlist,feeList) {
